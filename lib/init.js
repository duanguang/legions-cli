                ...
                switch (boilerplateName) {
                  ...
                  case 'boilerplate-weapp':
                    ...
                    case 'boilerplate-vue-nuxt-ssr':
                      ...
                      break;
                    case 'boilerplate-template-react':
                      inquirer.prompt([{
                        type:'input',
                        message:'请输入模块名称：',
                        name:'name',
                      }]).then(projectInfoAnswer=>{
                        const {sourceDir} = bilerplateInfo
                        co(async function(){
                          await shell.mkdir('template')
                          await download.download(sourceDir,'template')
                          await shell.cp('-R','template/src/home/.',projectInfoAnswer.name)
                          await shell.rm('-rf','template')
                        })
                      })
                    break;
                  default:
